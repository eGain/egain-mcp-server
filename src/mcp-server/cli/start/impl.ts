...
async function startStdio(flags: StartCommandFlags) {
  ...
  const { server } = createMCPServer({
    ...
    security: {
      oAuthUser: flags["o-auth-user"] ?? "",
      oAuthCustomer: flags["o-auth-customer"] ?? "",
      oAuthAnonymousCustomer: flags["o-auth-anonymous-customer"] ?? "",
...
async function startSSE(cliFlags: StartCommandFlags) {
  ...
  app.get("/sse", async (req, res) => {
    ...
    const flags: StartCommandFlags = {
      ...
      "o-auth-user": (req.headers["oAuthUser"] as string)
        ?? cliFlags["o-auth-user"],
      "o-auth-customer": (req.headers["oAuthCustomer"] as string)
        ?? cliFlags["o-auth-customer"],
      "o-auth-anonymous-customer":
        (req.headers["oAuthAnonymousCustomer"] as string)
          ?? cliFlags["o-auth-anonymous-customer"],
      "access-token": (req.headers["accessToken"] as string)
        ?? cliFlags["access-token"],
    };
